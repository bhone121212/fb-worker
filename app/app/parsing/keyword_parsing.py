...
from ..database.models import Content, Post, PostStat, User, Photo, auto_post, auto_share, auto_report, automation 
...
def parse_keyword(credentials, browser, task_keyword):  
    ...
    def share_post_to_feed(credentials, browser, auto_share_entry):
        ...
        try:
            ...
            try:
                last_element = browser.find_elements(By.XPATH, "//div[@role='dialog']//span[@data-ad-rendering-role= 'share_button'] | //div[@class='x8cjs6t x13fuv20 x178xt8z x16n37ib xq8finb x1y1aw1k xwib8y2']//div[@aria-label='Send this to friends or post it on your profile.'] | //span[@data-ad-rendering-role='share_button']")[-1]
            ...
            try:
                ...
                like_box = WebDriverWait(browser, 30).until(
                    ec.element_to_be_clickable((By.XPATH, "//div[@role='dialog']//span[@data-ad-rendering-role='like_button']"))
                )
            except TimeoutException:
                # If the first XPath fails, try the second one
                try:
                    like_box = WebDriverWait(browser, 30).until(
                        ec.element_to_be_clickable((By.XPATH, "//div[@role='dialog']//span[@data-ad-rendering-role='like_button'] | //div[@role='dialog']//div[@aria-label='Like']"))
                    )
                except TimeoutException:
            ...
            try:
                ...
                share_box = WebDriverWait(browser, 30).until(
                    ec.element_to_be_clickable((By.XPATH, "//div[@role='dialog']//span[@data-ad-rendering-role= 'share_button']"))
                )
            except TimeoutException:
                # If the first XPath fails, try the second one
                try:
                    share_box = WebDriverWait(browser, 30).until(
                        ec.element_to_be_clickable((By.XPATH, "//div[@role='dialog']//div[@aria-label='Send this to friends or post it on your profile.']"))
                    )
                except TimeoutException:
            ...
            if share_text:
                ...
                text_area = WebDriverWait(browser, 30).until(
                    ec.element_to_be_clickable((By.XPATH, "//div[@role='textbox'][@aria-placeholder='Say something about this...'] | //div[@class='x78zum5 xdt5ytf x1iyjqo2 x1n2onr6']//div[@role='textbox'] | //div[@class='xvq8zen x19f6ikt']//div[@role='textbox']"))
                )
                sleep(1)
    ...
    def report_post(credentials, browser, auto_report_entry):
        ...
        try:
            ...
            report_box = WebDriverWait(browser, 30).until(
                ec.presence_of_element_located((By.XPATH, "//div[contains(@class, 'x1qjc9v5 x78zum5')]//div[@aria-label='Actions for this post'] | //div[@aria-label='More']")))
            report_box.click()
            ...
            report_post = WebDriverWait(browser, 30).until(
                ec.presence_of_element_located((By.XPATH, "//span[contains(text(), 'who reported')]|//span[contains(text(), 'Report video')]"))
            )
            report_post.click()
            sleep(5)
...
def parse_post_keyword(browser, post, task_id, fb_date):
    def get_fb_post_id(browser, post):
        try:
    ...
    def get_user(post):
        ...
        try:
            ...
            try:
                user_element_1 = post.find_element(By.CSS_SELECTOR, 'span>a>strong>span[class], span>a[class]:not([aria-label])')
                ...
                user.name = user_element_1.text
            try:
                user_element_2 = post.find_element(By.CSS_SELECTOR, "span>a[class]:not([aria-label])")
                user.link = trim_full_link(user_element_2.get_attribute("href"))
                if not user.link:
                    raise NoSuchElementException
            except NoSuchElementException:
                user_element_2 = post.find_element(By.CSS_SELECTOR, "span[class='xt0psk2']>a[role='link'][tabindex='0'], span>a[role='link'][tabindex='0'], strong>span>a")
                user.link = trim_full_link(user_element_2.get_attribute("href"))                
    ...
    def get_reactions_count(post, browser):
        ...
        try:
            ...
            try:
                ...
                print("✅ Clicked reaction summary")
                popup_opened = True  # Mark that the popup is opened
                sleep(1)
            except:
                print("❌ Could not find or click reactions summary. Returning empty reactions.")
            ...
            try:
                ...
                print("✅ Reaction popup opened successfully.")
                sleep(1)
            except:
                print("❌ Reaction popup did not open. Returning empty reactions.")
                return reactions_dict
            ...
            """try:
                ...
                if len(more_buttons) >= 2:
                    ...
                    # 🔹 **Re-find reaction elements to avoid stale element reference**
                    reaction_popup = WebDriverWait(browser, 5).until(
                        EC.presence_of_element_located((By.XPATH, "//div[contains(@class, 'x1qjc9v5 x78zum5 xdt5ytf x1n2onr6')]"))
                    )
                else:
                    print("ℹ️ No 'More' button found, using visible reactions.")
            except:
                print("ℹ️ No 'More' button found, using visible reactions.")"""
            ...
            for container in reaction_containers:
                try:
                    ...
                    if reaction_type:
                        ...
                        print(f"✅ Found reaction: {reaction_type} - {reaction_count}")
        ...
        # Always attempt to close the popup if it was opened
            if popup_opened:
                ...
                print("❌ No Reactions for this post")
    ...
    def get_post_object(browser, post, task_id, fb_date):
        fb_post_id = get_fb_post_id(browser, post)
    ...
    post_obj = get_post_object(browser, post, task_id, fb_date)
