...
def normalize_time_string(time_string):
    """
    Adds spaces between numbers and units to normalize the time string for consistent parsing.
    E.g., '3h' -> '3 h', '4d' -> '4 d', '1w' -> '1 wk'.
    """
    # Add space between number and unit
    time_string = re.sub(r'(\d)([a-zA-Z])', r'\1 \2', time_string)

    # Normalize 'w' to 'wk' for weeks
    time_string = re.sub(r'\b(\d+) w\b', r'\1 wk', time_string, flags=re.IGNORECASE)
...
def parse_datetime(time_string):
    try:
        ...
        time_string = normalize_time_string(time_string.strip())
        ...
        if re.search(r"\byrs\b|\byr\b|\by\b|\by", time_string, re.IGNORECASE):
            digits = re.findall(r'\d+', time_string)
            if digits:
